...
declare const animated: WithExtend<CreateAnimated & NativeComponents>;
...
declare type AnimatedProp<T> = [T, T] extends [infer T, infer DT] ? [DT] extends [never] ? never : DT extends void ? undefined : DT extends ReadonlyArray<any> ? TransformArray extends DT ? AnimatedTransform : AnimatedStyles<DT> : DT extends object ? [AssignableKeys<DT, ViewStyle>] extends [never] ? DT : AnimatedStyle<DT> : DT | AnimatedLeaf<T> : never;
...
declare type AnimatedStyle<T> = [T, T] extends [infer T, infer DT] ? DT extends void ? undefined : [DT] extends [never] ? never : DT extends object ? {
    [P in keyof DT]: P extends 'transform' ? AnimatedTransform : AnimatedStyle<DT[P]>;
